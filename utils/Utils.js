...
export const uaHelp = new Message(
    ...
    new TextComponent(`&d/ua plot &f[&dinteger&f]&c&o* &7Alias: /plot`).setHover("show_text", `&dTP to a garden plot fast ${suggest}`).setClick("suggest_command", `/ua plot`), `\n`,
    new TextComponent(`&7${line}`)
)
...
// const ignoreKeys = ["emojis_custom", "ignoredUsers", "messages"];
// function checkDefaults(data, defaults) {
...
//                 data[key] = checkDefaults(data[key], defaults[key]);
//             }
//         }
//     }
...
//             if (!defaults.hasOwnProperty(key) && !ignoreKeys.includes(key)) {
//                 delete data[key];
//             }
//         }
//     }
...
//     try {
//         if (!FileLib.exists("UwUaddons", "data.json")) {
//             data.save()
//         }
//         let data = JSON.parse(FileLib.read("UwUaddons", "data.json"))
//         data = checkDefaults(data, jsonFile)
